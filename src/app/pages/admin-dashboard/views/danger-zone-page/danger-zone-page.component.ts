  ...
  readonly actions: DangerAction[] = [
    {
      ...
      description:
        'Inserta el contenido inicial del portfolio (perfil, proyectos demo, skills, experiencia, etc.) si la base de datos está vacía.',
      confirmTitle: 'Ejecutar seed inicial',
      confirmBody:
        'Se insertará el contenido base del portfolio. Si ya existen datos, la operación se omitirá de manera segura.',
    ...
    },
    {
      ...
      description:
        'Inserta un conjunto completo de contenido de demostración (perfil, proyectos, skills, etc.) reemplazando el existente.',
      confirmTitle: 'Seed de contenido demo',
      confirmBody:
        '<strong>⚠️ Atención:</strong> Esto insertará contenido de demostración. Úsalo solo en entornos de prueba.',
    ...
    },
    {
      ...
      description:
        'Inserta los 5 temas base si no existen aún. La operación es segura y se omite si ya hay temas.',
      confirmTitle: 'Ejecutar seed de temas',
      confirmBody:
        'Se intentará insertar los 5 temas predeterminados. Si ya existen temas, la operación se omitirá.',
      buttonLabel: 'Ejecutar seed',
    },
    {
      ...
      description:
        'Elimina TODOS los temas actuales y los reemplaza con los 5 predeterminados. Esta acción es irreversible.',
      confirmTitle: 'Forzar seed de temas',
      confirmBody:
        'Se eliminarán TODOS los temas actuales y se insertarán los 5 predeterminados. Esta acción no se puede deshacer.',
  ...
  async confirmAction(): Promise<void> {
    ...
    try {
      if (action.id === 'seed-themes' || action.id === 'seed-themes-force') {
        const result = await this.themeService.runSeedThemes(
          action.isForce ?? false,
        );
        if (result.seeded) {
          this.toastr.success(
            `Seed ejecutado: ${result.count} temas insertados.`,
            'Danger Zone',
          );
        } else {
          this.toastr.info(
            'Ya existen temas. Usa "Forzar seed" para reemplazarlos.',
            'Danger Zone',
          );
        }
        ...
        this.contentService.invalidateAllContentCache();
        this.toastr.success(
          'Seed de contenido demo ejecutado correctamente.',
          'Danger Zone',
        );
      }
    } catch {
